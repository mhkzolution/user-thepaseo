...
export default function BannerUploadTermPage() {
  const [banners, setBanners] = useState<BannerUploadTerm[]>([])
  ...
  useEffect(() => {
    ...
    try {
      const res = await fetch(`${API_URL}/banner/banneruploadterm`);
        ...
        const activeBanners = data
          .filter((banner: BannerUploadTerm) => {
            const endDate = banner.endDate ? new Date(banner.endDate) : null
            return banner.isActive && (!endDate || endDate >= now)
          })
          .sort((a: BannerUploadTerm, b: BannerUploadTerm) => a.order - b.order)
