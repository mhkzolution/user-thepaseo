...
export default function BannerPointPage() {
  const [banners, setBanners] = useState<BannerPoint[]>([])
  ...
  useEffect(() => {
    ...
    try {
      const res = await fetch(`${API_URL}/banner/bannerpoint`);
        ...
        const activeBanners = data
          .filter((banner: BannerPoint) => {
            const endDate = banner.endDate ? new Date(banner.endDate) : null
            return banner.isActive && (!endDate || endDate >= now)
          })
          .sort((a: BannerPoint, b: BannerPoint) => a.order - b.order)
  ...
  return (
    <div className="embla w-full rounded-2xl shadow-lg" ref={emblaRef}>
