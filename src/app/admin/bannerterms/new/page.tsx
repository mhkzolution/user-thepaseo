...
export default function BannerTermsNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/admin/bannerterms", {
      method: "POST",
    ...
    if (res.ok) router.push("/admin/bannerterms");
    ...
    <div>
      ...
      <Card className="bg-white">
        <CardContent className="md:p-10 p-4 space-y-4">
          <h1 className="text-2xl font-bold">เพิ่ม bannerterms ใหม่</h1>
          ...
          <Button className="bg-paseo" onClick={handleSubmit}>บันทึก bannerterms</Button>
        </CardContent>
      </Card>
    </div>
  );
