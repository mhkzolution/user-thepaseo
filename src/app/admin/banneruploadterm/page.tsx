...
export default function AdminbanneruploadPage() {
  ...
  // ดึงข้อมูลทั้งหมด
  useEffect(() => {
    fetch("/api/admin/banneruploadterm")
      .then((res) => res.json())
      .then((data) => {
        setBanners(data);
        setLoading(false);
  ...
  const moveBanner = async (fromIndex: number, toIndex: number) => {
    ...
    try {
      const res = await fetch("/api/admin/banneruploadterm/reorder", {
        method: "POST",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/admin/banneruploadterm/${id}`, { method: "DELETE" });
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/banneruploadterm/${id}/edit`);
  };
  ...
  return (
    <div className="p-0 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">จัดการ Banner เงื่อนไข (อัพโหลด)</h1>
        <Button
          className="bg-paseo hover:bg-paseo-hover"
          onClick={() => router.push("/admin/banneruploadterm/new")}
        >
