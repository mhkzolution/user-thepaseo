...
export default function BannerUploadTermNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/admin/banneruploadterm", {
      method: "POST",
    ...
    if (res.ok) router.push("/admin/banneruploadterm");
    ...
    <div>
      ...
      <Card className="bg-white">
        <CardContent className="md:p-10 p-4 space-y-4">
          <h1 className="text-2xl font-bold">เพิ่ม banneruploadterm ใหม่</h1>
          ...
          <Button className="bg-paseo" onClick={handleSubmit}>บันทึก bannerupload</Button>
        </CardContent>
      </Card>
    </div>
  );
