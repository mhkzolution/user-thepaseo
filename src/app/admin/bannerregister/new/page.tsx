...
export default function BannerRegisterNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/admin/bannerregister", {
      method: "POST",
    ...
    if (res.ok) router.push("/admin/bannerregister");
    ...
    <div>
      ...
      <Card className="bg-white">
        <CardContent className="md:p-10 p-4 space-y-4">
          <h1 className="text-2xl font-bold">เพิ่ม bannerregister ใหม่</h1>
          ...
          <Button className="bg-paseo" onClick={handleSubmit}>บันทึก bannerregister</Button>
        </CardContent>
      </Card>
    </div>
  );
