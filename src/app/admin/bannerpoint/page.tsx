...
export default function AdminbannerpointPage() {
  ...
  const moveBanner = async (fromIndex: number, toIndex: number) => {
    ...
    try {
      const res = await fetch("/api/admin/bannerpoint/reorder", {
        method: "POST",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/admin/bannerpoint/${id}`, { method: "DELETE" });
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/bannerpoint/${id}/edit`);
  };
      ...
      <div className="mb-10">
        ...
        <div className="space-y-3">
          {list.map((banner, idx) => {
            ...
            return (
              <Card key={banner.id} className="p-4 flex items-center justify-between gap-4">
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">จัดการ Banner collect</h1>
        <Button
          className="bg-paseo hover:bg-paseo-hover"
          onClick={() => router.push("/admin/bannerpoint/new")}
        >
