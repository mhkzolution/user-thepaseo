...
export default function BannerLoginNewPage() {
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/admin/bannerlogin", {
      method: "POST",
    ...
    if (res.ok) router.push("/admin/bannerlogin");
    ...
    <div>
      ...
      <Card className="bg-white">
        <CardContent className="md:p-10 p-4 space-y-4">
          <h1 className="text-2xl font-bold">เพิ่ม bannerlogin ใหม่</h1>
          ...
          <Button className="bg-paseo" onClick={handleSubmit}>บันทึก bannerlogin</Button>
        </CardContent>
      </Card>
    </div>
  );
