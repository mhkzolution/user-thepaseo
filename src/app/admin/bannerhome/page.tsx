...
export default function AdminBannerHomePage() {
  ...
  const moveBanner = async (fromIndex: number, toIndex: number) => {
    ...
    try {
      const res = await fetch("/api/admin/bannerhome/reorder", {
        method: "POST",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await fetch(`/api/admin/bannerhome/${id}`, { method: "DELETE" });
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/bannerhome/${id}/edit`);
  };
  ...
  return (
    <div className="p-0 md:p-6 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold">จัดการ Banner หน้าแรก</h1>
        <Button
          className="bg-paseo hover:bg-paseo-hover"
          onClick={() => router.push("/admin/bannerhome/new")}
        >
